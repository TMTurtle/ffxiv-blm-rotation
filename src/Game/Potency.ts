...
export class Potency {
	...
	getAmount(props: {
		...
		if (props.includePartyBuffs) {
			controller.game.getPartyBuffs(this.sourceTime).forEach(buff => {
	...
	getPartyBuffs() {
		return [...controller.game.getPartyBuffs(this.snapshotTime ?? 0).keys()];
	}
	...
	#calculateDamage(crit: number, dh: number, damageBonus?: number, critBonus?: number, dhBonus?: number) {
		...
		const critDamage = modifier * (1000 * this.#criticalHitStrength(crit)) / 1000;
		const dhDamage = modifier * 1250 / 1000;
